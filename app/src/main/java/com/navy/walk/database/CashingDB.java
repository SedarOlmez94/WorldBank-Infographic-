...
import static com.navy.walk.database.schema.DatabaseConstants.*;
...
public class CashingDB {
     ...
     */
    public CashingDB(Context context, DatabaseSchema schema) {
        this.schema = schema;
     ...
     * @return              A map of columns and values of the desired entry
     * @throws NotInSchemaException In case that the request was malformed
     */
    public Map<String,String> getWholeByPrimary(String tableName,Map<String,String> primaryKeys) throws NotInSchemaException{
        TableSchema table = checkTable(tableName, primaryKeys, true);

        List<Map<String,String>> ret = search(tableName, null, table.wherePrimary(),getSortedPrimary(primaryKeys,table), "1");
        if (ret.isEmpty())
            return null;
        return ret.get(0);
    }
     ...
     * @param primaryKeys   The map of primary keys for this table and their values
     * @return              List of rows that match this request
     * @throws NotInSchemaException
     */
    public List<Map<String,String>> getWholeByPrimaryPartial(String tableName,Map<String,String> primaryKeys) throws NotInSchemaException{
        ...
        return search(tableName, null, where, args, null);
    }
     ...
     * @return              A string that is the value of the desired column
     * @throws NotInSchemaException In case that the request was malformed
     */
    public String getColumnByPrimary(String tableName,Map<String,String> primaryKeys, String column) throws NotInSchemaException {
        TableSchema table = checkTable(tableName, primaryKeys, true);

        List<Map<String,String>> ret = search(tableName,new String[]{column},table.wherePrimary(),getSortedPrimary(primaryKeys, table),"1");
        if (ret.isEmpty())
            return null;
        return ret.get(0).get(column);
    }
     ...
     * @return  A list of column values for all the entries in the given table
     * @throws NotInSchemaException In case that the request was malformed
     */
    public List<String> getAllColumn(String tableName, String column) throws NotInSchemaException {
        if(!schema.hasTable(tableName))
            throw new NotInSchemaException();

        List<Map<String,String>> ret = search(tableName, new String[]{column}, null, null, null);
        List<String> result = new ArrayList<>();
        for (Map<String, String> stringStringMap : ret) {
            result.add(stringStringMap.get(column));
        }
        return result;
    }
     ...
     * @param tableName The name of the table which to search
     * @return A list of Maps that have columnName->value
     * @throws NotInSchemaException In case that the request was malformed
     */
    public List<Map<String,String>> getAll(String tableName) throws NotInSchemaException {
        return search(tableName,null,null, null, null);
    }

    /**
     * Gets all values that match a given primary key, this is to be used together with composite primary key, useless otherwise
     * @param tableName     The name of the table where to look for this row
     * @param primaryKeys   The map of primary keys for this table and their values
     * @param column        The desired column to fetch
     * @return A list of column values for all the entries in the given table that match the entry
     * @throws NotInSchemaException In case that the request was malformed
     */
    public List<String> getAllColumnByPrimary(String tableName,Map<String,String> primaryKeys, String column) throws NotInSchemaException {
        List<Map<String,String>> ret = getWholeByPrimaryPartial(tableName,primaryKeys);
        List<String> result = new ArrayList<>();
        for (Map<String, String> stringStringMap : ret) {
            result.add(stringStringMap.get(column));
        }
     ...
     * @throws NotInSchemaException In case that the request was malformed
     */
    private List<Map<String,String>> search(String tableName, String[] columns, String selection, String[] args, String limit) throws NotInSchemaException{
        //Table exists
        if (!schema.hasTable(tableName))
            throw new NotInSchemaException();
        ...
        //querying
        Cursor cursor = db.query(true, tableName, columns, selection, args, null, null, null, limit);
